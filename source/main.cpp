			...
			for (int i = 0; i < size_per_unit; ++i) {
				...
				if (count < utSize) {
					if (wiz::ToString(global->GetUserTypeList(utCount)->GetName()).empty()) {
						...
						value.push_back(wxVariant(wiz::ToString(global->GetUserTypeList(utCount)->GetName()).c_str()));
					}
					value.push_back(wxVariant(wxT("")));
					utCount++;
				}
				else {
					value.push_back(wxVariant(wiz::ToString(global->GetItemList(itCount).GetName()).c_str()));
					value.push_back(wxVariant(wiz::ToString(global->GetItemList(itCount).Get(0)).c_str()));
					itCount++;
				}
...
protected:
	...
	virtual void FileLoadMenuOnMenuSelection(wxCommandEvent& event) {
		...
		if (openFileDialog->ShowModal() == wxID_OK) {
			...
			global.Remove();
			wiz::load_data::LoadData::LoadDataFromFile3(fileName, global, 7, 8);
			now = &global;
	...
	virtual void m_code_run_buttonOnButtonClick(wxCommandEvent& event) { 
		...
		try {
			...
			wiz::ClauText().excute_module(mainStr, now, excuteData, opt, 0);
